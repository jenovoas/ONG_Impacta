...
describe('Auth + Tenant isolation (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    await db.organization.create({
      data: {
        ...
        plan: 'FREE',
        users: {
          create: { email: 'a@a.cl', passwordHash, role: 'SUPERADMIN' },
        },
      },
    });
    await db.organization.create({
      data: {
        ...
        plan: 'FREE',
        users: {
          create: { email: 'b@b.cl', passwordHash, role: 'SUPERADMIN' },
        },
      },
    });
    ...
    tokenA = (
      await request(app.getHttpServer())
        .post('/auth/login')
        .send({ email: 'a@a.cl', password: 'pw123456', orgSlug: 'org-a' })
    ).body.access_token;
    tokenB = (
      await request(app.getHttpServer())
        .post('/auth/login')
        .send({ email: 'b@b.cl', password: 'pw123456', orgSlug: 'org-b' })
    ).body.access_token;
  ...
  it('login rechaza password inválido', async () => {
    await request(app.getHttpServer())
      .post('/auth/login')
      .send({ email: 'a@a.cl', password: 'wrongpassword', orgSlug: 'org-a' })
      .expect(401);
  });
